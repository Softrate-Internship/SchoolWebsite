...
<h1 style="margin-left: 3%;">Edit Information</h1>
<br>
    ...
    if(isset($_POST["submit"])){
        ...
        if(isset($_FILES['file']['name'])){
            ...
            if($fileName!=""){
                ...
                $sql = "UPDATE news SET file = '$fileName' WHERE id=$id";
                $res=mysqli_query($conn,$sql);
            }
            else{
                if(isset($_POST['delpic'])){
                    unlink("uploadedFiles/news/".$f);
                    $sql = "UPDATE news SET file = '' WHERE id=$id";
                    $res=mysqli_query($conn,$sql);                                        
                }
            }
        }
        ...
        $sql = "UPDATE news SET heading = '$heading', body= '$body' WHERE id=$id";
        $res=mysqli_query($conn,$sql);
        if($res==1){ ?>
        <script>
            alert("Updated Succesfully");
            window.location.href='adminNews.php?name=<?php echo $name;?>';            
        </script>
        <?php 
        }
        else{ ?>
